...
int main(int argc, char ** argv){
	...
	int i, page_size, transfer_count ; 
	...
	transfer_count = write(fd, dummy_data, TRANSFER_SIZE);
	clock_gettime(CLOCK_REALTIME, &cpu_time);
	end_time = cpu_time.tv_nsec ;
	diff_time = end_time - start_time ;
	diff_time = diff_time/1000000000 ;
	printf("transfer_started at %d, ended at %d \n", start_time, end_time);
	printf("transffered %d bytes in %f s : %f B/s \n", transfer_count, diff_time, (transfer_count)/diff_time);
	printf("fifo has nb %d free places \n", ioctl(fd, PAPILIO_FIFO_NB_FREE));

	printf("fifo has nb %d available places \n", ioctl(fd, PAPILIO_FIFO_NB_AVAILABLE));

	printf("reading  %d bytes \n", TRANSFER_SIZE);
	clock_gettime(CLOCK_REALTIME, &cpu_time);
	start_time = cpu_time.tv_nsec ;
	transfer_count = read(fd, dummy_data, TRANSFER_SIZE);
	clock_gettime(CLOCK_REALTIME, &cpu_time);
	end_time = cpu_time.tv_nsec ;
	diff_time = end_time - start_time ;
	diff_time = diff_time/1000000000 ;
