...
The first project uses logicape_demo.vhd for top level design. This project is used to validate communication and evaluate bandwidth.
The architecture is made of a mux address interface, a fifo peripheral and an adressable latch. When writing to the fifo, the 16bit data is byte swapped (0xAA55 => 0x55AA) and written back to the output fifo. Thus when reading the fifo the design returns the byte swapped data. When writing to the latch the lsb control the logicape led1. Reading the latch returns push_button_1 value (no debouncing).

Demo setup :
pre-requisite: A logicape board, a beaglebone with demo image (http://downloads.angstrom-distribution.org/demo/beaglebone/) loaded onto sdcard. gcc installed on beaglebone (opkg install gcc).
FPGA side : 
1) compile the top level of the project and load the FPGA (project should not show error and some warning)
2) after loading nothing led(0) should be blinking on the logicape
Beaglebone side:
...
5) press push button 0 on the logicape (reset)
